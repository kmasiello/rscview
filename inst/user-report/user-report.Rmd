...
users_tbl <-
  ...
  mutate(days_since_active = as.numeric(today() - as_date(active_time))) %>%
  mutate(count_as_licensed_named_user = dplyr::case_when(
    as_date(active_time) >= (today() - dyears(1)) & locked == FALSE ~ TRUE, 
    TRUE ~ FALSE
  ))
# Could also accomplish the count_as_licensed_user feat by the endpoint and parameter 
```
...
lock_history <- logs %>% filter(action == "update_lock_user") %>% 
  ...
  separate(user, into = c(NA, "username", NA), sep = "[\\(\\)]") %>% 
  mutate(event = case_when(lock_state == "true" ~ "locked", 
...
# Additions by role
history_role <- add_history %>% left_join(select(users_tbl, username, user_role), by = "username")
