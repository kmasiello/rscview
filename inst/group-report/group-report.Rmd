...
```{r get_groups_tbl, include=FALSE}

conn <- create_connection()

groups_tbl <- get_groups_tbl(conn = conn)
...
```{r}
groups_summary <- groups_tbl %>% group_by(group_name) %>% 
...
make_table <- function(x) {
  reactable(
  filter(groups_tbl, group_name == x), searchable = TRUE, highlight = TRUE,
...
renderTable({
  filter(groups_tbl, username == input$username) %>%
