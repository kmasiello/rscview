...
shiny <- read_rds(here::here("inst", "usage-report", "shiny_2021-06-14.rds"))

shiny <- shiny %>% select(-ended)
...
#   limit = Inf
# )
other <- read_rds(here::here("inst", "usage-report", "static_2021-06-14.rds"))

other <- other %>% select(-variant_key, -rendering_id, -bundle_id) %>% 
  rename("started" = time)
